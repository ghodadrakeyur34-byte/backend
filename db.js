...
export async function insertListing(listing) {
  ...
  if (sb) {
    ...
    if (error) {
      console.error('[Supabase] Failed to write listing directly to external database:', error.message);
      throw new Error(`Database error: Failed to save listing directly to Supabase (${error.message})`);
    }
    console.log(`[Supabase] Successfully persisted listing ${listing.id} directly to external database.`);
  } else {
    console.warn('[Database] WARNING: Supabase not connected. Writing to local fallback.');
  }

  // Mirror to Firestore if configured
  const db = getFirestoreInstance();
  if (db) {
    try {
      await db.collection('listings').doc(String(listing.id)).set(listing);
    } catch (e) {
      console.warn('[Firestore] Failed to mirror listing:', e.message);
    }
  }

  // Write to local disk as backup
  try {
    const current = await listingsStore.read();
    const updated = [listing, ...current.filter(l => l.id !== listing.id)];
    await listingsStore.write(updated);
  } catch (e) {
    console.warn('[Store] Local file backup error:', e.message);
  }

  // Update in-memory cache
  if (cache.listings) {
    cache.listings = [listing, ...cache.listings.filter(l => l.id !== listing.id)];
  } else {
...
export async function updateListing(id, updates) {
  ...
  const updatedListing = { ...currentListings[index], ...updates };
  ...
  if (sb) {
    ...
    const { error } = await sb.from('listings').upsert(row);
    if (error) {
      console.error('[Supabase] Failed to update listing in external database:', error.message);
      throw new Error(`Database error: Failed to update listing in Supabase (${error.message})`);
    }
    console.log(`[Supabase] Successfully updated listing ${id} directly in external database.`);
  }
  ...
  if (db) {
    try {
      await db.collection('listings').doc(String(id)).set(updatedListing);
    } catch (e) {}
  }

  // Update local file backup
  try {
    const fileListings = await listingsStore.read();
    const fIdx = fileListings.findIndex(l => l.id === id);
    if (fIdx !== -1) {
      fileListings[fIdx] = updatedListing;
      await listingsStore.write(fileListings);
    }
  } catch (e) {}

  currentListings[index] = updatedListing;
  cache.listings = currentListings;
  cacheTimestamps.listings = Date.now();
...
export async function deleteListing(id) {
  ...
  if (sb) {
    ...
    if (error) {
      console.error('[Supabase] Failed to delete listing from external database:', error.message);
      throw new Error(`Database error: Failed to delete listing from Supabase (${error.message})`);
    }
    console.log(`[Supabase] Successfully deleted listing ${id} directly from external database.`);
  }
  ...
  try {
    const fileListings = await listingsStore.read();
    const filtered = fileListings.filter(l => l.id !== id);
    await listingsStore.write(filtered);
  } catch (e) {}
...
export async function insertInquiry(inquiry) {
  ...
  if (sb) {
    ...
    const { error } = await sb.from('inquiries').upsert(row);
    if (error) {
      console.error('[Supabase] Failed to insert inquiry:', error.message);
      throw new Error(`Database error saving inquiry: ${error.message}`);
    }
  }
  ...
  if (db) {
    try {
      await db.collection('inquiries').doc(String(inquiry.id)).set(inquiry);
    } catch (e) {}
  }

  try {
    const current = await inquiriesStore.read();
    await inquiriesStore.write([inquiry, ...current.filter(i => i.id !== inquiry.id)]);
  } catch (e) {}

  if (cache.inquiries) {
    cache.inquiries = [inquiry, ...cache.inquiries.filter(i => i.id !== inquiry.id)];
  } else {
    cache.inquiries = [inquiry];
  }
...
export async function saveListings(listings) {
  // Enforce external database write directly
  const sbSuccess = await saveSupabaseCollection('listings', listings);
  if (sbSuccess === false && getSupabaseInstance()) {
    throw new Error('Database error: Failed to persist listings to Supabase external database.');
  }
